...
inline double get_angle(const vec2 &a) {
...
int main() {
    ...
    while(true) {
        ...
        double angle_to_cp1 = get_angle(vec2(ang * pi / 180), cp[cp_id] - pod);
        cerr << "angle to cp1: " << angle_to_cp1 << '\n';

        int D = dist(pod, cp[cp_id]);
        double angle = ang * pi / 180 + angle_to_cp1;
        double angleV = get_angle(rotate(v, angle_to_cp1 * 180 / pi));
        double normV = v.len();
        ...
        if(res_thr < 0)     res_thr = 0;
        if(res_thr > 200)   res_thr = 200;
